...
class PatientRegistrationPage(BasePage):
    ...
    def has_progressed_past_name_step(self) -> bool:
        """Return True when the wizard moves away from the name step.

        The public demo can render slightly different wizard labels after the
        name step. This assertion avoids a meaningless `assert True` while
        still staying flexible enough for the shared O2 environment.
        """
        if not self.is_visible(self.GIVEN_NAME, timeout=3):
            return True
        ...
        return any(self.page_contains_text(text, timeout=2) for text in next_step_indicators)
